"""DocumentReader Python kit — ctypes bindings to DocumentReaderSDK.dll (lib/cpu/).

Role: thin native API for integrators. Used by app.py (Flask). Prefer ``python demo`` → HTTP → app.py
...
def start_new_session(options: dict | None = None) -> str:
    if _start_new_session is None:
        return '{"msg":"DocSDK_startNewSession not in this DocumentReaderSDK.dll"}'
...
def start_new_page(options: dict | None = None) -> str:
    if _start_new_page is None:
        return '{"msg":"DocSDK_startNewPage not in this DocumentReaderSDK.dll"}'
    opts = options or {}
    options_b = _b(json.dumps(opts, separators=(",", ":")))
    out = _out(options_b)
...
def unload() -> str:
    if _unload is None:
        return '{"msg":"DocSDK_unload not in this DocumentReaderSDK.dll"}'
    out = _out()
    _unload(out)
    return out.value.decode("utf-8", errors="replace")
